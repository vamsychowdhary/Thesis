function [X,fvec,X_spec,THD] = ssm_linear_ode(a,x0,E,t,fs,fsig,N,Rl,s_ind,e_ind)

% This function calculates the output voltage for given input signal and load
% resistance using ODE45 built inside MATLAB.
%
% Input Parameters
...
% ODE45 is run in loop to avoid high frequency noise issue
X1 = [q;x;u]; % State vector
for ii = 1:N-1
    tstep = [t(ii) t(ii+1)];
    [~,X2] = ode45(@linear_ode45,tstep,X1);
    X1 = X2(size(X2,1),:)';
    X(:,ii+1) = X1;
end
